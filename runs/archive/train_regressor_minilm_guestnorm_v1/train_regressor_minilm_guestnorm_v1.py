...
# Variant A: guest normalization (residual target)
...
def main():
    ap = argparse.ArgumentParser(description="Train MiniLM Ridge with guest normalization (residual target).")
    ...
    # CV with leakage-safe guest means
    splits = kfold_indices(len(df), n_splits=5, seed=SEED)
    r2s, rhos = [], []
    for tr, te in splits:
        tr_means, tr_global = compute_guest_means(y_log[tr], df.iloc[tr]["guest"])
        # Residual targets
        y_tr = y_log[tr] - np.array([tr_means.get(g, tr_global) for g in df.iloc[tr]["guest"]])
        y_te = y_log[te] - np.array([tr_means.get(g, tr_global) for g in df.iloc[te]["guest"]])

        reg = Ridge(alpha=ALPHA)
        reg.fit(emb[tr], y_tr)
        y_pred = reg.predict(emb[te])
        r2s.append(float(reg.score(emb[te], y_te)))
        rhos.append(float(spearmanr(y_te, y_pred).correlation))

    # Train final model on full residuals
    full_means, full_global = compute_guest_means(y_log, df["guest"]) 
    y_resid_full = y_log - np.array([full_means.get(g, full_global) for g in df["guest"]])
    reg = Ridge(alpha=ALPHA)
    reg.fit(emb, y_resid_full)

    # Save artifacts
    reg_path = here / "ridge_regressor_guestnorm_v1.pkl"
    ...
    report = here / "training_report_guestnorm_v1.md"
    lines = []
    lines.append("# Training Report — Guest Normalization (v1)\n")
    lines.append(f"CSV: {args.csv}")
    lines.append(f"Model: {MODEL_NAME} | Dim: {dim} | Device: {device}")
    lines.append("")
    lines.append("## Cross-validated metrics (on residual target)\n")
    ...
    lines.append("- Guest means embedded in pickle (guest_means, guest_global_mean)")
    report.write_text("\n".join(lines))

    manifest = {
        "run": "train_regressor_minilm_guestnorm_v1",
        "csv": args.csv,
        "model_name": MODEL_NAME,
        "alpha": ALPHA,
        "dim": dim,
    ...
    print("Training complete (guest normalization).")
