...
MODEL_NAME = "sentence-transformers/all-MiniLM-L6-v2"  # chosen via v2 benchmark
BATCH_SIZE = 128
SEED = 42
ALPHA = 1.0
...
def main():
    ...
    data_csv = here.parent.parent / "data" / "training-data_v2.csv"
    ...
    emb_path = here / f"shorts_embeddings_all-MiniLM-L6-v2_{dim}_v2.parquet"
    emb_df = pd.DataFrame({
        ...
        "config_version": "train_regressor_minilm_v2",
    })
    ...
    reg = Ridge(alpha=ALPHA)
    reg.fit(emb, y)
    model_path = here / "ridge_regressor_v2.pkl"
    ...
    report = here / "training_report_v2.md"
    lines = []
    lines.append("# Regression Head Training Report (v2)\n")
