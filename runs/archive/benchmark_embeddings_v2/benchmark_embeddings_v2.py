...
def write_embeddings_parquet(df: pd.DataFrame, embeddings: np.ndarray, model_name: str, out_dir: Path) -> Path:
    ...
    out_df = pd.DataFrame({
        ...
        "config_version": "benchmark_v2",
    })
    out_df["embedding"] = [e.astype("float32") for e in embeddings]

    slug = model_slug(model_name)
    out_path = out_dir / f"shorts_embeddings_{slug}_{dim}_v2.parquet"
...
def main():
    ...
    report_path = here / "embedding_benchmark_v2.md"
    ...
    lines = []
    lines.append(f"# Embedding Benchmark v2\n")
